...
namespace BeyondIndustry.Factory
{
    ...
    public class T_Traeger_Ecke : FactoryMachine
    {
        public float Height { get; set; } = 1.0f;
        public Color TintColor { get; set; } = Color.White;
        
        public T_Traeger_Ecke(Vector3 position, Model model, float height = 3.0f) 
            : base(position, model)
        {
        ...
        public class Provider : IMachineProvider
        {
            public List<MachineDefinition> GetDefinitions(Model defaultModel)
            {
                ...
                beamDef.CreateMachineFunc = (pos) => new T_Traeger_Ecke(pos, beamDef.Model, 3.0f);
